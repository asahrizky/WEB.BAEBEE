...
namespace WEB.BAEBEE.Core.Notification.Query
{
    ...
    internal class GetNotificationCountHandler : IRequestHandler<GetNotificationCountRequest, ObjectResponse<int>>
    {
        ...
        public async Task<ObjectResponse<int>> Handle(GetNotificationCountRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var data = await _context.Entity<WEB.BAEBEE.Data.Model.Notification>().Where(d => d.IdUser == request.IdUser && d.IsOpen == request.IsOpen).CountAsync();
                result.Data = data;
                result.OK();
            }
            catch (Exception ex)
